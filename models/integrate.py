...
from module.ME_Modules import MEModule, MeGemPModule
...
__all__ = ['ME_CoordAtt_MultiLoss_ResNet50', ]

# base
class ResNet_Video_Insert(nn.Module):
    def __init__(self, last_stride=1, block=Bottleneck, layers=[3, 4, 6, 3], insert_layers=[0, 2, 3, 1]):
        self.inplanes = 64
        super().__init__()
        ...
        me_idx = 0
        self.Ins_1 = nn.ModuleList([MeGemPModule(self.inplanes)
                                    for i in range(insert_layers[me_idx])])
        self.Ins_1_idx = sorted([layers[0]-(i+1)
                                 for i in range(insert_layers[me_idx])])
        me_idx += 1

        self.layer2 = self._make_layer(block, 128, layers[1], stride=2)
        self.Ins_2 = nn.ModuleList([MeGemPModule(self.inplanes)
                                    for i in range(insert_layers[me_idx])])
        ...
        self.Ins_3 = nn.ModuleList([MeGemPModule(self.inplanes)
                                    for i in range(insert_layers[me_idx])])
        ...
        self.Ins_4 = nn.ModuleList([CoordAtt(self.inplanes)
                                    for i in range(insert_layers[me_idx])])
...
class ME_CoordAtt_MultiLoss_ResNet50(nn.Module):
    def __init__(self, num_classes):
        super(ME_CoordAtt_MultiLoss_ResNet50, self).__init__()
        # 0231 90.2 85.4
        self.backbone = ResNet_Video_Insert(insert_layers=[0, 2, 3, 1])
