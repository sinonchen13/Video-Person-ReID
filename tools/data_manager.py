...
class Mars(object):
    ...
    def __init__(self, root='/data/datasets/', min_seq_len=0):
        ...
        self.track_train_info_path = osp.join(
            self.root, 'info/tracks_train_info.mat')
        self.track_test_info_path = osp.join(
            self.root, 'info/tracks_test_info.mat')
        ...
        query, num_query_tracklets, num_query_pids, num_query_imgs = \
            self._process_data(test_names, track_query, home_dir='bbox_test',
                               relabel=False, min_seq_len=min_seq_len)

        gallery, num_gallery_tracklets, num_gallery_pids, num_gallery_imgs = \
            self._process_data(test_names, track_gallery, home_dir='bbox_test',
                               relabel=False, min_seq_len=min_seq_len)
        ...
        num_total_tracklets = num_train_tracklets + \
            num_gallery_tracklets + num_query_tracklets
        ...
        print("  train    | {:5d} | {:8d}".format(
            num_train_pids, num_train_tracklets))
        print("  query    | {:5d} | {:8d}".format(
            num_query_pids, num_query_tracklets))
        print("  gallery  | {:5d} | {:8d}".format(
            num_gallery_pids, num_gallery_tracklets))
        print("  ------------------------------")
        print("  total    | {:5d} | {:8d}".format(
            num_total_pids, num_total_tracklets))
        print("  number of images per tracklet: {} ~ {}, average {:.1f}".format(
...
class DukeMTMCVidReID(object):
    ...
    def __init__(self, root='/data/datasets/', min_seq_len=0, verbose=True, **kwargs):
        ...
        self.split_train_dense_json_path = osp.join(
            self.dataset_dir, 'split_train_dense.json')
        ...
        train_dense, num_train_tracklets_dense, num_train_pids_dense, num_imgs_train_dense = \
            self._process_dir_dense(
                self.train_dir, self.split_train_dense_json_path, relabel=True, sampling_step=64)
        query, num_query_tracklets, num_query_pids, num_imgs_query = \
            self._process_dir(
                self.query_dir, self.split_query_json_path, relabel=False)
        gallery, num_gallery_tracklets, num_gallery_pids, num_imgs_gallery = \
            self._process_dir(self.gallery_dir,
                              self.split_gallery_json_path, relabel=False)

        print("the number of tracklets under dense sampling for train set: {}".format(
        ...
        for pdir in pdirs:
            ...
            for tdir in tdirs:
                ...
                for img_idx in range(num_imgs):
                    ...
                    if len(res) == 0:
                        print("Warn: index name {} in {} is missing, jump to next".format(
                            img_idx_name, tdir))
                        continue
    ...
    def _process_dir_dense(self, dir_path, json_path, relabel, sampling_step=32):
        ...
        for pdir in pdirs:
            ...
            for tdir in tdirs:
                ...
                # dense sampling  一段超长的tracklet，变成好几段sampling_step长度的 tracklet
                num_sampling = len(img_paths)//sampling_step
                if num_sampling == 0:  # 片段长度小于num_sampling 直接全部加入
                    tracklets.append((img_paths, pid, camid))
                else:
                    for idx in range(num_sampling):
                        if idx == num_sampling - 1:
                            ...
                            tracklets.append(
                                (img_paths[idx*sampling_step: (idx+1)*sampling_step], pid, camid))
...
class LSVID(object):
    ...
    def __init__(self, root='/data/datasets/', min_seq_len=0):
        ...
        self.train_list_path = osp.join(
            self.root, 'list_sequence/list_seq_train.txt')
        self.test_frames_path = osp.join(
            self.root, 'list_frame/list_lvreid_test.txt')
        train_names = self._get_names_train(self.train_list_path)
        test_names = self._get_names_test(self.test_frames_path, self.root)
        self.info_dir = osp.join(self.root, 'test/data')
        ...
        num_train_pids = []
        num_train_tracklets = len(train_names)
        num_train_imgs = []

        for train_name in train_names:

            img_paths = glob.glob(
                osp.join(self.root, train_name.split(" ")[0]+'*'))
            img_paths = tuple(img_paths)
            pid = int(train_name.split(" ")[1])
            camid = int(osp.basename(img_paths[0]).split(".")[0].split("_")[2])
            train_tracklets.append((img_paths, pid, camid))
            num_train_pids.append(pid)
            num_train_imgs.append(len(img_paths))

        test_info = h5py.File(osp.join(self.info_dir, "info_test.mat"))[
            "info_test"]
        test_query_IDX = h5py.File(
            osp.join(self.info_dir, "info_test.mat"))["query"]

        test_info = np.array(test_info).T
        test_query_IDX = np.array(test_query_IDX).T
        test_info[:, 0:2] -= 1
        test_query_IDX -= 1
        test_query_IDX = [i for i in range(
            test_info.shape[0]) if i in test_query_IDX]
        test_gallery_IDX = [i for i in range(
            test_info.shape[0]) if i not in test_query_IDX]
        track_query = test_info[test_query_IDX, :]
        track_gallery = test_info[test_gallery_IDX, :]

        num_gallery_pids = []
        num_gallery_tracklets = track_gallery.shape[0]
        num_gallery_imgs = []

        for i in range(track_gallery.shape[0]):

            # start_frame_idx ,end_frame_idx, pid,camid
            start_idx, end_idx, pid, camid = track_gallery[i, :]
            # print(pid)
            pid = int(pid)
            camid = int(camid)
            ...
            img_paths = test_names[start_idx:end_idx+1]
            img_paths = tuple(img_paths)
            gallery_tracklets.append((img_paths, pid, camid))
            num_gallery_pids.append(pid)
            num_gallery_imgs.append(len(img_paths))

        num_query_pids = []
        num_query_tracklets = track_query.shape[0]
        num_query_imgs = []

        for i in range(track_query.shape[0]):

            # start_frame_idx ,end_frame_idx, pid,camid
            start_idx, end_idx, pid, camid = track_query[i, :]
            pid = int(pid)
            camid = int(camid)
            start_idx = int(start_idx)
            end_idx = int(end_idx)
            ...
            img_paths = test_names[start_idx:end_idx+1]
            img_paths = tuple(img_paths)
            query_tracklets.append((img_paths, pid, camid))

            num_query_pids.append(pid)
            num_query_imgs.append(len(img_paths))

        self.train = train_tracklets
        self.query = query_tracklets
        self.gallery = gallery_tracklets

        num_train_pids = len(set(num_train_pids))
        num_query_pids = len(set(num_query_pids))
        num_gallery_pids = len(set(num_gallery_pids))
    ...
    def _get_names_train(self, fpath):
        names = []
        with open(fpath, 'r') as f:
            for line in f:
                new_line = line.rstrip()
    ...
    def _get_names_test(self, fpath, root):
        names = []
        with open(fpath, 'r') as f:
            for line in f:
                new_line = line.rstrip()
                new_line = new_line.split(" ")[0]
                names.append(root+'/'+new_line)
        return names
...
class iLIDSVID(object):
    ...
    def __init__(self, root='/data/datasets/', split_id=9):
        ...
        if split_id >= len(splits):
            raise ValueError("split_id exceeds range, received {}, but expected between 0 and {}".format(
        ...
        train_dense, _, _, _ = self._process_train_data(
            train_dirs, cam1=True, cam2=True)
        query, num_query_tracklets, num_query_pids, num_imgs_query = \
    ...
    def _prepare_split(self):
        if not osp.exists(self.split_path):
            ...
            print("Totally {} splits are created, following Wang et al. ECCV'14".format(
                len(splits)))
            ...
            if cam1:
                ...
                if num_sampling == 0:
                    ...
                    for idx in range(num_sampling):
                        if idx == num_sampling - 1:
                            tracklets.append(
                                (img_names[idx*sampling_step:], pid, 1))
                        else:
                            tracklets.append(
                                (img_names[idx*sampling_step: (idx+2)*sampling_step], pid, 1))
...
class prid(object):
    ...
    def __init__(self, root='/data/datasets/', split_id=9):
        ...
        self.data_dir = osp.join(self.root, 'prid_2011')

        self.split_path = osp.join(self.root, 'splits_prid2011.json')
        self.cam_1_path = osp.join(self.root, 'prid_2011/multi_shot/cam_a')
        self.cam_2_path = osp.join(self.root, 'prid_2011/multi_shot/cam_b')
